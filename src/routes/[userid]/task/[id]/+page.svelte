<script lang="ts">
	...
	import { selectAction, selected } from '$lib/store/store';
	...
	let isLoading = false;
	let selectedTab = 0;
	let selectedAction = act_types[0];

	selected.subscribe((value) => {
		selectedTab = value;
	});

	selectAction.subscribe((value) => {
								...
								<div class="flex flex-col items-center flex-shrink-0">
									<div class="w-10 h-10 flex items-center justify-center">
										<div class={TASKTYPEICON[el?.type].className}>
											<Icon src={TASKTYPEICON[el?.type].icon} size={TASKTYPEICON[el?.type].size} />
										</div>
									</div>
